...
.. |armarmv5| replace:: https://developer.arm.com/docs/ddi0100/latest/armv5-architecture-reference-manual
.. |armarmv7| replace:: https://developer.arm.com/docs/ddi0406/c/arm-architecture-reference-manual-armv7-a-and-armv7-r-edition

+--------------------------+-----------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
| Ref                      | URL or other external reference                                                                     | Title                                                                                                             |
+==========================+=====================================================================================================+===================================================================================================================+
| :doc:`AAELF <aaelf32>`   |                                                                                                     | ELF for the Arm Architecture.                                                                                     |
+--------------------------+-----------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
| :doc:`BSABI <bsabi32>`   |                                                                                                     | ABI for the Arm Architecture (Base Standard)                                                                      |
+--------------------------+-----------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
| :doc:`CPPABI <cppabi32>` |                                                                                                     | C++ ABI for the Arm Architecture                                                                                  |
+--------------------------+-----------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
| :doc:`EHABI <ehabi32>`   |                                                                                                     | Exception Handling ABI for the Arm Architecture. (*This document*)                                                |
+--------------------------+-----------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
...
.. glossary::
   ...
   ABI
      ...
      2. A particular aspect of the specifications to which independently
         ...
         linkable and executable.  For example, the :doc:`C++ ABI for the
         Arm Architecture <cppabi32>`, the :doc:`Run-time ABI for the Arm
         Architecture <rtabi32>`, the :doc:`C Library ABI for the Arm
...
The ABI for the Arm architecture ([:doc:`BSABI <bsabi32>`]) specifies four generic
...
run-time system in a program, as depicted in :ref:`ehabi32-fig1`, below.

.. _ehabi32-fig1:

.. figure:: /images/ehabi32-run-time-calls.png
...
across such interfaces in :ref:`ehabi32-fig1`, above.
...
functions described in :ref:`ehabi32-section8` that implement generic C++
...
A design principle underlying ELF [:doc:`AAELF <aaelf32>`] can be caricatured
...
table entry (see :ref:`ehabi32-section5` and :ref:`ehabi32-section6` for details).

There are two table entry formats (again see :ref:`ehabi32-section6` for details).
...
specific formats where possible (see :ref:`ehabi32-section9`).
...
must be SHT\_ARM\_EXIDX (see [:doc:`AAELF <aaelf32>`]). It must have the SHF\_LINK\_ORDER
...
object files. [:doc:`AAELF <aaelf32>`] and platform-ABI documents contain further details
...
to [:doc:`AAELF <aaelf32>`] for additional details):
...
It is mandatory that the machinery listed in :ref:`ehabi32-section4-6` is implemented using
...
while searching the stack, see :ref:`ehabi32-section7`) they can be saved just before they
...
-  The first word contains a prel31 offset (see :ref:`ehabi32-section4-4-2`) to the start of
...
-  The second word contains one of:
   ...
   - The exception-handling table entry itself with bit 31 set, if it can
     be encoded in 31 bits (see :ref:`ehabi32-section6-3`).
   ...
   - The special bit pattern EXIDX\_CANTUNWIND (0x1), indicating to
     ...
     :ref:`ehabi32-section7-3` and :ref:`ehabi32-section7-4`.

.. note::

   - It is essential that link-time symbol vectoring (see [:doc:`AAELF <aaelf32>`]) does
...
The language-independent unwinding library described in :ref:`ehabi32-section7` calls the
...
unwound and its exception-handling table entry (see :ref:`ehabi32-section7-2`).
...
.. code-block:: c
  ...
  /* See :ref:`ehabi32-section7-2` for details of the various _Unwind types */
...
.. figure:: /images/ehabi32-table-entry-generic.png
...
.. figure:: /images/ehabi32-table-entry-arm.png
...
emitted inline in the index table instead (as described in :ref:`ehabi32-section5`). Bit 31
...
Arm has allocated index numbers 0, 1 and 2 for use by C and C++. :ref:`ehabi32-section9`
...
-  The content of exception index table entries (:ref:`ehabi32-section5`) and the
   language-independent first word of exception-handling table entries
   (:ref:`ehabi32-section6`).
...
described in detail in :ref:`ehabi32-section7-3` and :ref:`ehabi32-section7-4`.
...
:ref:`ehabi32-section3-1`.

The UCB is defined as part of this ABI - see :ref:`ehabi32-section7-2`.
...
are described separately in :ref:`ehabi32-section7-5`; the runtime representation is opaque
...
-  \_US\_VIRTUAL\_UNWIND\_FRAME Used in phase 1. See :ref:`ehabi32-section7-3`.

-  \_US\_UNWIND\_FRAME\_STARTING Used in phase 2. See :ref:`ehabi32-section7-4`.

-  \_US\_UNWIND\_FRAME\_RESUME Used in phase 2. See :ref:`ehabi32-section7-4`.
...
the personality routine. See :ref:`ehabi32-section7-5`.
...
-  Exception\_cleanup is used to support multi-language environments and
   to delete objects that are no longer required. See :ref:`ehabi32-section7-6`.
...
-  Barrier\_cache is reserved for use by the language semantics library
   ...
   not explicitly claimed by the semantics library routines). See :ref:`ehabi32-section7-3`
   and :ref:`ehabi32-section7-4`.
...
\_Unwind\_RaiseException begins a new exception propagation. See :ref:`ehabi32-section7-3`.

\_Unwind\_Resume resumes an existing exception propagation after
execution of a cleanup. See :ref:`ehabi32-section7-4`.
...
housekeeping. The details are implementation-defined, but see :ref:`ehabi32-section7-7`.

\_Unwind\_DeleteException is described in :ref:`ehabi32-section7-6`.
...
an ECO, and will then (from C++ via \_\_cxa\_throw, see :ref:`ehabi32-section8-4`) call the
...
(these can later be deallocated by \_Unwind\_Complete – see :ref:`ehabi32-section7-7` for
further remarks).
...
1. The index table is searched for the entry E that matches the return
   ...
   contains the special bitpattern EXIDX\_CANTUNWIND (see :ref:`ehabi32-section5`), the
...
1. At the start of each new frame, \_Unwind\_Next\_Frame is entered with
   ...
   EXIDX\_CANTUNWIND (see :ref:`ehabi32-section5`), the unwinder will call abort(). Otherwise
...
a consequence of throwing an exception (see :ref:`ehabi32-section11` for a further discussion
...
:ref:`ehabi32-section7-1-3`. The size and content of the LEO are private to the semantics
...
-  This asserts there are N type\_info object references available, at
   ...
   :ref:`ehabi32-section4-4-2`). \_\_cxa\_call\_unexpected will know how to follow these to
...
.. note::
   ...
   re-thrown. See :ref:`ehabi32-section8-5-4`.
...
See :ref:`ehabi32-section8-5-4` for remarks on use of this routine.
...
.. note::
   ...
   extracted before making that call. See :ref:`ehabi32-section7-7` for more details.
...
handler the return value is undefined. See :ref:`ehabi32-section8-5-4` for remarks on use of
this routine.
...
\_Unwind\_DeleteException. See :ref:`ehabi32-section7-6` and :ref:`ehabi32-section8-6`.
...
barrier\_cache.bitpattern fields must contain data as described in :ref:`ehabi32-section8-2`
...
The first word is as described in :ref:`ehabi32-section6-3`. The most significant byte
...
.. admonition:: Aside
   ...
   As stated in :ref:`ehabi32-section6-3`, object producers must emit an
...
(described in :ref:`ehabi32-section9-3`) that can be interpreted byte by byte. There are two
formats.
...
(see :ref:`ehabi32-section4-4-2`) to a landing pad, with bit 31 clear.
...
table entry. :ref:`ehabi32-section9-4` explains how the personality routine processes
a table.
...
.. note::
   ...
   phase 1 is to call terminate(). See :ref:`ehabi32-section7-3` and :ref:`ehabi32-section7-4` for more details.
...
:ref:`ehabi32-table4`, below, describes the Arm-defined frame-unwinding instructions
...
-  The following remarks clarify the table:
  ...
  a) Code 0x80 0x00, *Refuse to unwind*, causes the PR to return
     ...
     :ref:`ehabi32-section5`) but it permits descriptors in the function body.
...
(see :ref:`ehabi32-section8-4`), save whatever it needs in the UCB cleanup\_cache (see :ref:`ehabi32-section7-2`)
...
for a match against the catch type. \_\_cxa\_type\_match (see :ref:`ehabi32-section8-4`) may
...
\_\_cxa\_type\_match (see :ref:`ehabi32-section8-4`) may be used. No match against any of the
...
-  If it does, the PR should set the VRS for entry to that pad (passing
   ...
   \_\_cxa\_call\_unexpected is in the barrier\_cache (see :ref:`ehabi32-section8-2`), and
...
-  If it does not, the PR should execute the unwind description to
   ...
   \_\_cxa\_call\_unexpected (see :ref:`ehabi32-section8-4`), ensure the data required for
   \_\_cxa\_call\_unexpected is in the barrier\_cache (see :ref:`ehabi32-section8-2`), then
...
In such cases, void terminate() is called (18.6.3). In the situation
   ...
   terminate() is called. An implementation is not permitted to
   finish stack unwinding prematurely based on a determination that
   the unwind process will eventually cause a call to terminate().
...
the C++ implementation, and the unwinding control block (UCB—see :ref:`ehabi32-section7-2`).
...
-  *implementation\_ever\_called\_terminate* is true if the unwind
   ...
   correct implementation of std::uncaught\_exception(). See :ref:`ehabi32-section11` for a
   lengthy discussion).
...
-  *caughtExceptions* is required by re-throwing using *throw* with no
   ...
   :ref:`ehabi32-section11` for a lengthy discussion.
